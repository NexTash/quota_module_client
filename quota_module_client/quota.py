...
def validate_files_space_limit():
    ...
    allowed_space = quota.get("space")

    # allowed space value type check
    if type(allowed_space) is not int:
        frappe.throw(
            _("Invalid value for maximum Space limit. It can be a whole number only."), frappe.ValidationError)
    ...
    # Updating in DB
    frappe.db.sql(
        f"""UPDATE `tabQ M` SET used_space={total_size}, private_files_size={private_files_size}, public_files_size={public_files_size}, backup_files_size={backup_files_size} WHERE id = 1""")
...
def validate_db_space_limit():
    ...
    # Site config
    quota = get_quota()
    allowed_db_space = quota.get("db_space")

    # allowed DB space value type check
    if type(allowed_db_space) is not int:
        frappe.throw(
            _("Invalid value for maximum Database Space limit. it can be a whole number only."), frappe.ValidationError)
    ...
    frappe.db.sql(
        f"""UPDATE `tabQ M` SET used_db_space={used_db_space} WHERE id = 1""")
...
def company_limit(self, method):
    ...
    if type(allowed_companies) is not int:
        frappe.throw(
            _("Invalid value for maximum allowed Companies limit. it can be a whole number only."), frappe.ValidationError)

    # Calculating total companies
    total_company = len(frappe.db.get_all('Company', filters={}))

    frappe.db.sql(
        f"""UPDATE `tabQ M` SET used_company={total_company} WHERE id = 1""")

    # Validation
    if allowed_companies != 0 and total_company >= allowed_companies:
        if not frappe.get_list('Company', {'name': self.name}):
            frappe.throw(_("Only {} company(s) allowed and you have {} company(s).Please remove other company or to increase the limit please contact sales").format(
                quota.get('company'), total_company))
